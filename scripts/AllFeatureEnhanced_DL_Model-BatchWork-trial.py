...
def dpp_loss(features_set, gamma=1.0, epsilon=1e-5):
    """
    Calculates the DPP-based diversity loss for a set of feature vectors.

    Args:
        features_set (torch.Tensor): Tensor of shape (N_q, E) representing N_q feature vectors,
                                     each of dimension E.
        gamma (float): Parameter for the RBF kernel.
        epsilon (float): Small value for numerical stability in log_determinant.

    Returns:
        torch.Tensor: A scalar tensor representing the DPP loss.
    """
    if features_set.dim() != 2 or features_set.size(0) <= 1: # N_q <= 1 or incorrect dimensions
        return torch.tensor(0.0, device=features_set.device, dtype=features_set.dtype)

    # Pairwise squared Euclidean distances
    # features_set: (N_q, E)
    # diff: (N_q, N_q, E) where diff[i,j,:] = features_set[i,:] - features_set[j,:]
    diff = features_set.unsqueeze(1) - features_set.unsqueeze(0)
    # dist_sq: (N_q, N_q) where dist_sq[i,j] = ||features_set[i,:] - features_set[j,:]||^2
    dist_sq = torch.sum(diff ** 2, dim=2)

    # RBF kernel (Similarity matrix L)
    # L: (N_q, N_q)
    L = torch.exp(-gamma * dist_sq)

    # Add epsilon for numerical stability before taking determinant
    L_stable = L + epsilon * torch.eye(L.size(0), device=L.device, dtype=L.dtype)

    # Calculate log determinant
    _sign, log_det_L = torch.linalg.slogdet(L_stable)
...
class MultiQueryAttentionPooling(nn.Module):
    def __init__(self, input_dim, num_pooling_queries=4, num_heads=2, dropout=0.2):
        ...
        self.input_dim = input_dim # 保存input_dim以便在返回空张量时使用
        self.self_attn = nn.MultiheadAttention(embed_dim=input_dim, num_heads=num_heads, dropout=dropout, batch_first=True)
    ...
    def forward(self, features, key_padding_mask=None):
        batch_size = features.size(0)
        queries_expanded = self.queries.expand(batch_size, -1, -1)
        
        if features.size(1) == 0: # Handle empty sequences
            # 返回与期望输出形状匹配的零张量
            empty_pooled = torch.zeros(batch_size, self.num_pooling_queries * self.input_dim, device=features.device)
            empty_attn_output_raw = torch.zeros(batch_size, self.num_pooling_queries, self.input_dim, device=features.device)
            return empty_pooled, empty_attn_output_raw # MODIFIED: Return two outputs

        # attn_output_raw will be (B, num_pooling_queries, E)
        # 这是我们想要应用DPP损失的输出
        attn_output_raw, _ = self.self_attn(query=queries_expanded, key=features, value=features, key_padding_mask=key_padding_mask, need_weights=False)
        
        # 为主要任务展平多个查询输出: (B, num_pooling_queries * E)
        pooled = attn_output_raw.contiguous().view(batch_size, -1)
        pooled = self.dropout_mod(pooled)
        
        # MODIFIED: 同时返回池化（展平）输出和原始（未展平）注意力输出
        return pooled, attn_output_raw
...
class _5_6TCRRepModel_Revised(nn.Module):
    def __init__(self, embedding_dim=960, dropout=[0.2,0.2,0.4], num_classes=4,
                 ...
                 aggregation_type='single_query_attention', 
                 num_pooling_queries=4,
                 # +++ START: DPP Loss hyperparameters +++
                 use_dpp_loss=False,       # 控制是否使用DPP损失的标志
                 dpp_loss_weight=0.1,    # DPP损失的权重 (lambda_DPP)
                 dpp_gamma=1.0           # DPP中RBF核的gamma参数
                 # +++ END: DPP Loss hyperparameters +++
                ):
        super().__init__()
        ...
        # +++ START: Store DPP Loss related attributes +++
        self.use_dpp_loss = use_dpp_loss
        self.dpp_loss_weight = dpp_loss_weight
        self.dpp_gamma = dpp_gamma
        # +++ END: Store DPP Loss related attributes +++
        ...
        if self.aggregation_type == 'single_query_attention':
            self.pooling_module = SingleQueryAttentionPooling(hidden_dim, dropout=dropout[1], num_heads=num_heads)
            self.class_head_input_dim = hidden_dim
        elif self.aggregation_type == 'multi_query_attention':
            # MultiQueryAttentionPooling 现在返回两个输出
            self.pooling_module = MultiQueryAttentionPooling(hidden_dim, num_pooling_queries=self.num_pooling_queries, dropout=dropout[1], num_heads=num_heads)
        ...
        self.class_head = nn.Sequential(
            ...
            nn.Linear(self.class_head_input_dim, self.class_head_input_dim // 2 if self.class_head_input_dim > 1 else 1),
            nn.GELU(),
    ...
    def forward(self, inputs, attention_mask=None, film_condition_data=None, clone_fractions_for_film=None):
        ...
        current_batch_dpp_loss = torch.tensor(0.0, device=inputs.device, dtype=inputs.dtype)
        # +++ END: Initialize DPP loss +++
        
        x = self.feat_proj_linear(inputs)
        # ... (feat_proj, FiLM conditioning, CLS token, Transformer layers 的代码保持不变) ...
        if K_orig > 0:
        ...
        if self.aggregation_type == 'cls_token':
            ...
            if attention_mask is not None:
                ...
                effective_token_mask = torch.cat((cls_mask, attention_mask), dim=1) 
                final_key_padding_mask = ~effective_token_mask 
            elif K_orig == 0 and x.shape[1] == 0 : 
                final_x_for_transformer = cls_tokens
                final_key_padding_mask = torch.zeros(B,1,dtype=torch.bool, device=x.device)
        elif attention_mask is not None: 
            final_key_padding_mask = ~attention_mask 
        
        processed_seqs = final_x_for_transformer
        for layer_module in self.transformer_layers:
            sa_input = layer_module['norm_sa'](processed_seqs)
            attn_output_tf, _ = layer_module['self_attn']( # Renamed to avoid confusion
                query=sa_input, key=sa_input, value=sa_input,
                key_padding_mask=final_key_padding_mask,
                need_weights=False
            )
            processed_seqs = processed_seqs + layer_module['dropout_sa'](attn_output_tf)
            ffn_input = layer_module['norm_ffn'](processed_seqs)
            ffn_output = layer_module['ffn'](ffn_input)
            processed_seqs = processed_seqs + layer_module['dropout_ffn'](ffn_output)
            
        aggregated_features = None
        # +++ START: Variable to store the raw attention output for DPP +++
        attn_output_raw_for_dpp_loss_calc = None
        # +++ END: Variable for DPP +++

        if self.aggregation_type == 'cls_token':
            aggregated_features = processed_seqs[:, 0] 
        else: 
            sequences_to_pool = processed_seqs
            mask_for_pooling = final_key_padding_mask

            if self.cls_token is not None and final_x_for_transformer.shape[1] == K_orig +1 : 
                sequences_to_pool = processed_seqs[:, 1:] 
                if final_key_padding_mask is not None:
                     mask_for_pooling = final_key_padding_mask[:, 1:] 

            pool_input_normed = self.norm_pool(sequences_to_pool)
            
            # MODIFIED: Handle output from pooling_module
            if self.aggregation_type == 'multi_query_attention':
                aggregated_features, attn_output_raw_for_dpp_loss_calc = self.pooling_module(pool_input_normed, key_padding_mask=mask_for_pooling)
            elif self.aggregation_type == 'single_query_attention':
                 aggregated_features = self.pooling_module(pool_input_normed, key_padding_mask=mask_for_pooling)
                 # attn_output_raw_for_dpp_loss_calc will remain None

        # +++ START: Calculate DPP loss if enabled +++
        if self.training and self.use_dpp_loss and self.aggregation_type == 'multi_query_attention' and \
           attn_output_raw_for_dpp_loss_calc is not None and attn_output_raw_for_dpp_loss_calc.size(0) > 0: # Check batch size > 0
            
            # attn_output_raw_for_dpp_loss_calc has shape (B, N_q, E)
            if attn_output_raw_for_dpp_loss_calc.size(1) > 1: # N_q > 1 for diversity
                batch_dpp_losses_list = []
                for i in range(attn_output_raw_for_dpp_loss_calc.size(0)): # Iterate over batch
                    # Get features for the i-th sample: (N_q, E)
                    sample_features_for_dpp = attn_output_raw_for_dpp_loss_calc[i]
                    
                    # Optional: Normalize features before DPP loss calculation
                    # sample_features_for_dpp = torch.nn.functional.normalize(sample_features_for_dpp, p=2, dim=1)

                    loss_val = dpp_loss(sample_features_for_dpp, gamma=self.dpp_gamma)
                    batch_dpp_losses_list.append(loss_val)
                
                if batch_dpp_losses_list: # if list is not empty
                   current_batch_dpp_loss = torch.stack(batch_dpp_losses_list).mean()
        # +++ END: Calculate DPP loss +++

        final_classification_output = self.class_head(aggregated_features)

        # MODIFIED: Return main output and auxiliary DPP loss during training
        if self.training and self.use_dpp_loss and self.aggregation_type == 'multi_query_attention':
            return final_classification_output, current_batch_dpp_loss
        else:
            # For consistency in unpacking, always return two values, DPP loss can be None or 0.0
            return final_classification_output
...
def main(args): # args 现在是 current_args 的角色
    ...
    if args.load_model_path is None and train_dataset:
        ...
        if active_labels and len(active_labels) > 0 : # Ensure active_labels is not empty
            bincount_size = max(max(active_labels) + 1 if active_labels else 0, 
                                np.max(train_original_labels_np) + 1 if len(train_original_labels_np) > 0 else 0)
            if bincount_size > 0:
                class_sample_count = np.bincount(train_original_labels_np, minlength=bincount_size)
                active_class_sample_count = class_sample_count[active_labels]
                print(f"训练数据类别分布 (活动标签 {active_labels}): {dict(zip(active_labels, active_class_sample_count))}")
                if not np.all(active_class_sample_count == 0):
                    active_class_weights = 1. / (active_class_sample_count + 1e-6)
                    full_class_weights = np.zeros(bincount_size)
                    full_class_weights[active_labels] = active_class_weights
                    sample_weights = full_class_weights[train_original_labels_np]
                    sampler = WeightedRandomSampler(weights=sample_weights, num_samples=len(train_dataset), replacement=True)
                else: print("警告: 所有活动类别的样本数均为0，无法计算权重。")
            else: print("警告: 无法确定类别用于加权采样 (bincount_size=0)。")
        else: print("警告: 无活动标签，使用标准随机采样。")
    ...
    model = _5_6TCRRepModel_Revised(
        embedding_dim=embedding_dim, num_classes=actual_num_classes, hidden_dim=args_for_run.hidden_dim, # Changed args to args_for_run
        num_heads=args_for_run.num_heads, num_transformer_layers=args_for_run.num_transformer_layers,
        film_condition_source=args_for_run.film_condition_source,
        num_gene_types=num_gene_types,
        film_mlp_internal_dim=args_for_run.film_mlp_internal_dim,
        aggregation_type=args_for_run.aggregation_type,
        num_pooling_queries=args_for_run.num_pooling_queries,
        dropout=args_for_run.dropout,
        # +++ Pass DPP parameters +++
        use_dpp_loss=args_for_run.use_dpp_loss,
        dpp_loss_weight=args_for_run.dpp_loss_weight,
        dpp_gamma=args_for_run.dpp_gamma
    ).to(device)
    ...
    if args.load_model_path:
        ...
        for epoch in range(args.epochs):
            ...
            for batch_data_train in pbar:
                ...
                optimizer.zero_grad()
                # MODIFIED: Get outputs from model
                primary_outputs = None
                aux_dpp_loss_value = None # Initialize

                # Model forward pass
                # The model now always returns two values if training for consistency
                model_outputs = model(
                    embeddings_b, 
                    attention_mask=attention_masks_b, 
                    film_condition_data=film_condition_data_for_model_train, 
                    clone_fractions_for_film=clone_fractions_for_film_model_train
                )
                
                primary_outputs = model_outputs[0]
                if model.training and model.use_dpp_loss and model.aggregation_type == 'multi_query_attention':
                    aux_dpp_loss_value = model_outputs[1]

                loss = torch.tensor(0.0, device=device) 
                if label_map and mapped_labels_b.numel() > 0 and primary_outputs is not None:
                    main_task_loss = criterion(primary_outputs, mapped_labels_b)
                    loss = main_task_loss

                    # Add DPP loss if applicable
                    if aux_dpp_loss_value is not None and model.dpp_loss_weight > 0:
                        loss = loss + model.dpp_loss_weight * aux_dpp_loss_value
                        pbar.set_postfix({'loss': main_task_loss.item(), 'dpp_loss': aux_dpp_loss_value.item() * model.dpp_loss_weight}) # Show both
                    else:
                        pbar.set_postfix({'loss': main_task_loss.item()})

                if loss.requires_grad: # Only backward if loss requires grad (e.g., not a zero tensor if no labels)
                    loss.backward()
                    optimizer.step()
                ...
                if primary_outputs is not None:
                    scores_b = torch.softmax(primary_outputs, dim=1)
                    preds_mapped_indices_b = torch.argmax(primary_outputs, dim=1)
...
if __name__ == "__main__":
    ...
    cli_parser.add_argument('--config_path', type=str, default="/xiongjun/test/NEW-MIL-dir-backup/backup/NEW-MIL-dir-backup/exp_json/521-exp-setup/521setup-p2.json",
    ...
    for i, config_params_from_json in enumerate(all_configurations):
        ...
        original_defaults_parser.add_argument('--use_dpp_loss', default=False, help='是否对MultiQueryAttention的输出使用DPP多样性损失') # Use action for bool
        original_defaults_parser.add_argument('--dpp_loss_weight', type=float, default=0.1, help='DPP损失的权重')
        original_defaults_parser.add_argument('--dpp_gamma', type=float, default=1.0, help='DPP损失中RBF核的gamma参数')
